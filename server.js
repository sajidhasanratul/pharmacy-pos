...
// Prevent index.html from caching so frontend additions are fetched immediately
...
const initDB = async () => {
  ...
  // Auto-migration check: If database already exists but lacks the "image", "tag", "brand" or "generic" columns, add them.
  try {
    ...
    const brandColumns = await dbQuery(`SHOW COLUMNS FROM products LIKE 'brand'`);
    if (brandColumns.length === 0) {
      await dbQuery(`ALTER TABLE products ADD COLUMN brand VARCHAR(255) DEFAULT NULL`);
      console.log('Database Migration: Added "brand" column to products table.');
    }
    const genericColumns = await dbQuery(`SHOW COLUMNS FROM products LIKE 'generic'`);
    if (genericColumns.length === 0) {
      await dbQuery(`ALTER TABLE products ADD COLUMN generic VARCHAR(255) DEFAULT NULL`);
      console.log('Database Migration: Added "generic" column to products table.');
    }
    const qtyPerUnitColumns = await dbQuery(`SHOW COLUMNS FROM variations LIKE 'qty_per_unit'`);
    if (qtyPerUnitColumns.length === 0) {
      await dbQuery(`ALTER TABLE variations ADD COLUMN qty_per_unit INT DEFAULT 1`);
      console.log('Database Migration: Added "qty_per_unit" column to variations table.');
    }
    const createdAtColumns = await dbQuery(`SHOW COLUMNS FROM products LIKE 'createdAt'`);
    if (createdAtColumns.length === 0) {
      await dbQuery(`ALTER TABLE products ADD COLUMN createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP`);
      console.log('Database Migration: Added "createdAt" column to products table.');
    }
    const costPriceCols = await dbQuery(`SHOW COLUMNS FROM orderItems LIKE 'costPrice'`);
    if (costPriceCols.length === 0) {
      await dbQuery(`ALTER TABLE orderItems ADD COLUMN costPrice DECIMAL(10,2) DEFAULT NULL`);
      console.log('Database Migration: Added "costPrice" column to orderItems table.');
    }
    const isCustomCols = await dbQuery(`SHOW COLUMNS FROM orderItems LIKE 'isCustom'`);
    if (isCustomCols.length === 0) {
      await dbQuery(`ALTER TABLE orderItems ADD COLUMN isCustom TINYINT(1) DEFAULT 0`);
      console.log('Database Migration: Added "isCustom" column to orderItems table.');
    }
  } catch (migErr) {
...
const seedCatalog = async () => {
  ...
  const cats = [
    ['cat-1', 'Analgesics & Antipyretics'],
    ['cat-2', 'Antihistamines'],
    ['cat-3', 'Gastrointestinal'],
    ['cat-4', 'Cardiovascular'],
    ['cat-5', 'Antidiabetics'],
    ['cat-6', 'Antibiotics']
  ];
  for (const c of cats) {
  ...
  // Add sample product
  await dbQuery(`INSERT INTO products (id, name, sku, barcode, brand, generic, costPrice, sellingPrice, stock, alertQty) VALUES 
    ('p1', 'Napa Extra', 'NAPA-001', '8901234567890', 'Beximco', 'Paracetamol', 0.80, 1.20, 560, 100),
    ('p2', 'Alatrol', 'ALA-001', '8901234568001', 'Square', 'Cetirizine', 20.00, 30.00, 225, 30),
    ('p3', 'Losec 20mg', 'LOS-001', '8901234569001', 'AstraZeneca', 'Omeprazole', 3.50, 5.00, 150, 50)`);

  await dbQuery(`INSERT INTO variations (id, productId, name, sku, barcode, price, costPrice, stock) VALUES 
    ('v1a', 'p1', 'Box of 100', 'NAPA-B100', '8901234567891', 120.00, 80.00, 5),
    ('v1b', 'p1', 'Strip of 10', 'NAPA-S10', '8901234567892', 12.00, 8.00, 60),
    ('v2a', 'p2', 'Box of 150', 'ALA-B150', '8901234568002', 450.00, 300.00, 1),
    ('v2b', 'p2', 'Strip of 10', 'ALA-S10', '8901234568003', 30.00, 20.00, 75)`);
...
app.get('/api/products', async (req, res) => {
  try {
    ...
    const prods = await dbQuery(`SELECT * FROM products ORDER BY createdAt DESC, id DESC`);
...
app.post('/api/products', verifyRole(['admin', 'manager']), async (req, res) => {
  try {
    const { id, name, sku, barcode, brand, generic, costPrice, sellingPrice, stock, alertQty, image, tag, status, priority, variations } = req.body;
    ...
    const brandVal = brand !== undefined ? brand : null;
    const genericVal = generic !== undefined ? generic : null;
    ...
    await dbQuery(`INSERT INTO products (id, name, sku, barcode, brand, generic, costPrice, sellingPrice, stock, alertQty, image, tag, status, priority) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, nameVal, skuVal, barcodeVal, brandVal, genericVal, costVal, sellVal, stockVal, alertVal, imageVal, tagVal, statusVal, priorityVal]);

    if (variations && variations.length > 0) {
      for (const v of variations) {
        const vSku = v.sku !== undefined ? v.sku : null;
        const vBarcode = v.barcode !== undefined ? v.barcode : null;
        const vQtyPerUnit = parseInt(v.qty_per_unit || v.qtyPerUnit || 1) || 1;
        const vPrice = sellVal * vQtyPerUnit;
        const vCost = costVal * vQtyPerUnit;
        const vStock = 0;

        await dbQuery(`INSERT INTO variations (id, productId, name, sku, barcode, price, costPrice, stock, qty_per_unit) 
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [v.id, id, v.name, vSku, vBarcode, vPrice, vCost, vStock, vQtyPerUnit]);
      }
    }
    res.json({ success: true });
  } catch (err) {
...
app.post('/api/products/bulk', verifyRole(['admin', 'manager']), async (req, res) => {
  try {
    ...
    for (const item of products) {
      const { name, sku, barcode, brand, generic, costPrice, sellingPrice, stock, alertQty, tag, variations } = item;
      if (!sku) continue;
      ...
      if (prodRows.length > 0) {
        ...
        await dbQuery(`UPDATE products SET name = ?, brand = ?, generic = ?, costPrice = ?, sellingPrice = ?, stock = ?, alertQty = ?, tag = ? WHERE id = ?`,
          [name || `Product ${sku}`, brand || null, generic || null, costPrice || 0, sellingPrice || 0, stock || 0, alertQty || 5, tag || null, pid]);
      } else {
        pid = 'prod_' + Math.random().toString(36).substr(2, 9);
        // Insert new product
        await dbQuery(`INSERT INTO products (id, name, sku, barcode, brand, generic, costPrice, sellingPrice, stock, alertQty, image, tag) 
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '', ?)`,
          [pid, name || `Product ${sku}`, sku, barcode || null, brand || null, generic || null, costPrice || 0, sellingPrice || 0, stock || 0, alertQty || 5, tag || null]);
      }
      ...
      if (variations && variations.length > 0) {
        ...
        for (const v of variations) {
          ...
          const vQtyPerUnit = parseInt(v.qty_per_unit || v.qtyPerUnit || 1) || 1;
          const vPrice = (sellingPrice || 0) * vQtyPerUnit;
          const vCost = (costPrice || 0) * vQtyPerUnit;
          const vStock = 0;

          await dbQuery(`INSERT INTO variations (id, productId, name, sku, barcode, price, costPrice, stock, qty_per_unit) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [vid, pid, v.name || 'Default', v.sku, v.barcode || null, vPrice, vCost, vStock, vQtyPerUnit]);

          totalPcsStock += (parseInt(v.stock) || 0) * vQtyPerUnit;
        }

        // Save total variations stock converted to pieces as the main product stock only if greater than 0
        if (totalPcsStock > 0) {
          await dbQuery(`UPDATE products SET stock = ? WHERE id = ?`, [totalPcsStock, pid]);
        }
      }
    }
...
app.put('/api/products/:id', verifyRole(['admin', 'manager']), async (req, res) => {
  try {
    ...
    const { name, sku, barcode, brand, generic, costPrice, sellingPrice, stock, alertQty, image, tag, status, priority, variations } = req.body;
    ...
    await dbQuery(`UPDATE products SET name = ?, sku = ?, barcode = ?, brand = ?, generic = ?, costPrice = ?, 
      sellingPrice = ?, stock = ?, alertQty = ?, image = ?, tag = ?, status = ?, priority = ? WHERE id = ?`,
      [nameVal, skuVal, barcodeVal, brandVal, genericVal, costVal, sellVal, stockVal, alertVal, imageVal, tagVal, statusVal, priorityVal, pid]);
    ...
    if (variations && variations.length > 0) {
      for (const v of variations) {
        ...
        await dbQuery(`INSERT INTO variations (id, productId, name, sku, barcode, price, costPrice, stock, qty_per_unit) 
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [v.id, pid, v.name, vSku, vBarcode, vPrice, vCost, vStock, vQtyPerUnit]);
      }
    }
    res.json({ success: true });
  } catch (err) {
...
app.post('/api/products/bulk-stock-update', verifyRole(['admin', 'manager', 'cashier']), async (req, res) => {
  try {
    ...
    for (const update of updates) {
      ...
      if (type === 'product') {
        await dbQuery(`UPDATE products SET stock = GREATEST(0, stock + ?) WHERE id = ?`, [amount, id]);
      } else if (type === 'variation') {
        const varRows = await dbQuery(`SELECT productId, qty_per_unit FROM variations WHERE id = ?`, [id]);
        if (varRows.length > 0) {
          const mult = parseInt(varRows[0].qty_per_unit) || 1;
          await dbQuery(`UPDATE products SET stock = GREATEST(0, stock + ?) WHERE id = ?`, [amount * mult, varRows[0].productId]);
        }
      }
    }
...
// Custom Order Items management
app.get('/api/custom-order-items', async (req, res) => {
  try {
    res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate');
    const rows = await dbQuery(`
      SELECT oi.id, oi.orderId, oi.productId, oi.productName, oi.variationName, oi.qty, oi.unitPrice, oi.total, oi.costPrice, oi.isCustom,
             o.invoiceId, DATE_FORMAT(o.date, '%Y-%m-%dT%H:%i:%s.000Z') as date, o.customerName, o.customerPhone
      FROM orderItems oi
      JOIN orders o ON oi.orderId = o.id
      WHERE oi.isCustom = 1 OR oi.productId LIKE 'custom_%'
...
app.put('/api/custom-order-items/:id', verifyRole(['admin', 'manager', 'cashier']), async (req, res) => {
  try {
    const itemId = req.params.id;
    const costPrice = req.body.costPrice !== undefined && req.body.costPrice !== null && req.body.costPrice !== '' 
      ? parseFloat(req.body.costPrice) 
      : null;
    await dbQuery(`UPDATE orderItems SET costPrice = ? WHERE id = ?`, [costPrice, itemId]);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.put('/api/custom-order-items-bulk', verifyRole(['admin', 'manager', 'cashier']), async (req, res) => {
  try {
    const { updates } = req.body;
    if (!Array.isArray(updates)) {
    ...
    for (const u of updates) {
      const costVal = u.costPrice !== undefined && u.costPrice !== null && u.costPrice !== '' ? parseFloat(u.costPrice) : null;
      await dbQuery(`UPDATE orderItems SET costPrice = ? WHERE id = ?`, [costVal, u.id]);
    }
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.delete('/api/orders/:id', verifyRole(['admin']), async (req, res) => {
  ...
  try {
    ...
    // 1. Get order items to restore stocks
    const items = await connection.execute(`SELECT * FROM orderItems WHERE orderId = ?`, [oid]);
    const orderItems = items[0] || [];

    for (const item of orderItems) {
      const isCustom = item.isCustom || (item.productId && item.productId.startsWith('custom_'));
      if (!isCustom) {
        let restoreQty = item.qty;
        if (item.variationName) {
          const [vars] = await connection.execute(`SELECT qty_per_unit FROM variations WHERE productId = ? AND name = ?`, [item.productId, item.variationName]);
          if (vars && vars.length > 0) {
            restoreQty = item.qty * (vars[0].qty_per_unit || 1);
          }
        }
        await connection.execute(`UPDATE products SET stock = stock + ? WHERE id = ?`, [restoreQty, item.productId]);
      }
    }

    // 2. Delete payments, orderItems, and the order itself
...
app.post('/api/orders', async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      const isCustom = item.isCustom || (item.productId && item.productId.startsWith('custom_')) ? 1 : 0;
      const costVal = item.costPrice !== undefined && item.costPrice !== null && item.costPrice !== '' ? parseFloat(item.costPrice) : null;

      await connection.execute(`INSERT INTO orderItems (id, orderId, productId, productName, variationName, qty, unitPrice, total, costPrice, isCustom) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [item.id, order.id, item.productId, item.productName, item.variationName, item.qty, item.unitPrice, item.total, costVal, isCustom]);
      ...
      if (!isCustom) {
        ...
        if (item.variationName) {
          const [vars] = await connection.execute(`SELECT qty_per_unit FROM variations WHERE productId = ? AND name = ?`, [item.productId, item.variationName]);
          if (vars && vars.length > 0) {
            deductQty = item.qty * (vars[0].qty_per_unit || 1);
          }
        }
        await connection.execute(`UPDATE products SET stock = stock - ? WHERE id = ?`, [deductQty, item.productId]);
      }
    }
...
app.put('/api/orders/:id', verifyRole(['admin', 'manager']), async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      ...
      await connection.execute(`INSERT INTO orderItems (id, orderId, productId, productName, variationName, qty, unitPrice, total, costPrice, isCustom) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [item.id, oid, item.productId, item.productName, item.variationName, item.qty, item.unitPrice, item.total, costVal, isCustom]);
...
app.post('/api/backup/import', verifyRole(['admin']), async (req, res) => {
  ...
  try {
    ...
    for (const p of products || []) {
      const pBrand = p.brand !== undefined ? p.brand : null;
      const pGeneric = p.generic !== undefined ? p.generic : (p.categoryId || null);
      await connection.execute(`INSERT INTO products (id, name, sku, barcode, brand, generic, costPrice, sellingPrice, stock, alertQty, image) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [p.id, p.name, p.sku, p.barcode || null, pBrand, pGeneric, p.costPrice, p.sellingPrice, p.stock, p.alertQty, p.image || null]);
    }
    for (const v of variations || []) {
