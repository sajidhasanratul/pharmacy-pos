  ...
  const EditOrder = {
    ...
    async render(orderId) {
      ...
      this.cart = items.map(i => {
        const isCustom = i.isCustom || (i.productId && i.productId.startsWith('custom_'));
        ...
        return {
          ...
          image: p ? p.image : '',
          stock: isCustom ? 999999 : stock,
          isCustom: !!isCustom,
          costPrice: i.costPrice !== undefined ? i.costPrice : null
        };
      });
      ...
      mc.innerHTML = `
        <div class="page-header fade-in">
          <div>
            ...
            <p class="page-subtitle">Modify the medicines, pricing, or payment splits for Invoice ${this.activeOrder.invoiceId}.</p>
          </div>
    ...
    renderCart() {
      ...
      this.cart.forEach((item, index) => {
        const total = (typeof item.unitPrice === 'number' ? item.unitPrice : (parseFloat(item.unitPrice) || 0)) * item.qty;
        const row = document.createElement('tr');
        row.className = 'cart-item-row';
        row.dataset.index = index;

        const thumbHtml = item.image 
          ? `<img src="${item.image}" style="width:32px; height:32px; border-radius:var(--radius-xs); object-fit:cover; border:1px solid var(--border);">` 
          : `<div style="width:32px; height:32px; border-radius:var(--radius-xs); background:var(--primary-50); display:flex; align-items:center; justify-content:center; font-size:11px; font-weight:700; color:#0d9488;">${item.isCustom ? '+' : 'Rx'}</div>`;

        const nameColHtml = item.isCustom
          ? `<div style="display:flex; align-items:center; gap:8px;">
               ${thumbHtml}
               <div style="flex:1;">
                 <input type="text" class="form-input input-custom-name" value="${H.esc(item.productName || '')}" placeholder="Enter medicine / item name..." style="padding:4px 8px; height:28px; font-size:12px; font-weight:600; width:100%;">
               </div>
             </div>`
          : `<div style="display:flex; align-items:center; gap:8px;">
               ${thumbHtml}
        ...
        const priceColHtml = item.isCustom
          ? `<div style="display:flex; align-items:center; gap:4px; max-width:110px;">
               <span>৳</span>
               <input type="number" class="input-price form-input" value="${item.unitPrice !== '' ? item.unitPrice : ''}" placeholder="0.00" min="0" step="0.01" style="padding:4px 6px; height:28px; width:100%; font-size:12px; font-weight:600;">
             </div>`
          : `<div style="display:flex; align-items:center; gap:4px; max-width:110px;">
               <span>৳</span>
        ...
        row.innerHTML = `
          ...
          <td class="cart-row-total" style="font-weight:700; color:var(--text-dark);">${H.formatCurrency(total)}</td>
          <td><button class="btn btn-danger btn-sm btn-qty-remove" style="padding:4px 8px; display:inline-flex; align-items:center; justify-content:center;" title="Remove Item"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button></td>
        `;
        ...
        if (priceInput) {
          priceInput.oninput = (e) => {
            ...
            if (totalCell) {
              totalCell.textContent = H.formatCurrency((typeof val === 'number' ? val : 0) * item.qty);
            }
            this.recalculate();
          };
        }
    ...
    async saveChanges() {
      ...
      for (const item of this.cart) {
        if (item.isCustom && (!item.productName || item.productName.trim() === '')) {
          H.showToast('Please enter a medicine or item name for all custom products.', 'warning');
          return;
        }
        const p = parseFloat(item.unitPrice);
        if (isNaN(p) || p <= 0) {
          H.showToast(`Please enter a valid selling price for "${item.productName || 'custom item'}".`, 'warning');
          return;
        }
      }

      const salesDate = document.getElementById('sales-date').value;
      const subtotal = this.cart.reduce((s, i) => s + ((parseFloat(i.unitPrice) || 0) * i.qty), 0);
      ...
      const orderItemsList = this.cart.map(item => ({
        ...
        total: (parseFloat(item.unitPrice) || 0) * item.qty,
