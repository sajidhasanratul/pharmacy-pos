  ...
  const Reports = {
    ...
    productProfitReport(container, orders, orderItems, products, H) {
      ...
      items.forEach(item => {
        ...
        let costPrice = 0;
        if (item.costPrice !== undefined && item.costPrice !== null && item.costPrice !== '' && !isNaN(parseFloat(item.costPrice))) {
    ...
    async exportReportCSV(type, from, to) {
      ...
      if (type === 'daily-sale') {
        ...
        payments.forEach(p => {
          ...
          pmtTotals[m] = (pmtTotals[m] || 0) + (parseFloat(p.amount) || 0);
        });
        csvData = Object.entries(pmtTotals).map(([Method, Amount]) => ({ Method, Amount }));
        ...
        csvData = products.map(p => ({
          ...
          Generic: p.generic || '',
          SKU: p.sku,
          'Total Pieces': p.stock,
          'Cost Price': p.costPrice,
          'Selling Price': p.sellingPrice,
          'Value (Cost)': p.stock * p.costPrice,
          'Value (Retail)': p.stock * p.sellingPrice
        }));
      } else if (type === 'sales-summary') {
        const totalSales = orders.reduce((s, o) => s + (parseFloat(o.grandTotal) || 0), 0);
        const totalDiscount = orders.reduce((s, o) => s + (parseFloat(o.discountAmount) || 0), 0);
        const totalTax = orders.reduce((s, o) => s + (parseFloat(o.taxAmount) || 0), 0);
        ...
        const totalReturned = orders.reduce((s, o) => s + (parseFloat(o.returnedAmount) || 0), 0);
