  ...
  const Router = {
    ...
    _resolve() {
      ...
      if (currentUser) {
        ...
        if (role === 'cashier') {
          const permitted = ['/new-order', '/sales-list', '/custom-item-prices', '/sales-return', '/return-list', '/inventory-details', '/bulk-stock-update', '/gallery'];
    ...
    _updateBreadcrumb(path) {
      ...
      const labels = parts.map(p => {
        ...
        if (p === 'payment-methods') return 'Payment Methods';
        if (p === 'invoice-settings') return 'Invoice Settings';
