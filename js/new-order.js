      ...
      mc.innerHTML = `
        <div class="page-header fade-in">
          <div>
            ...
            <p class="page-subtitle">Add customer details, search medicines, and process checkout.</p>
          </div>
        ...
        if (e.key === 'Enter') {
          ...
          for (const p of products) {
            if (p.variations && p.variations.length > 0) {
              ...
              if (vMatch) {
                const calculatedStock = Math.floor(p.stock / (vMatch.qty_per_unit || 1));
                const calculatedPrice = vMatch.price || (p.sellingPrice * (vMatch.qty_per_unit || 1));
                matchedItem = {
                  productId: p.id,
                  productName: p.name,
                  variationName: vMatch.name,
                  unitPrice: parseFloat(calculatedPrice),
                  qty: 1,
                  image: p.image,
                  stock: calculatedStock
                };
    ...
    renderCart() {
      ...
      this.cart.forEach((item, index) => {
        const thumbHtml = item.image 
          ...
          : `<div style="width:30px; height:30px; border-radius:var(--radius-xs); background:#f1f5f9; display:flex; align-items:center; justify-content:center; margin-right:8px; font-size:11px; font-weight:700; color:#0d9488;">${item.isCustom ? '+' : 'Rx'}</div>`;
        ...
        tbody.innerHTML += `
          <tr class="cart-item-row" data-index="${index}">
            ...
            <td class="cart-row-total" style="font-weight:700;">${H.formatCurrency((typeof item.unitPrice === 'number' ? item.unitPrice : (parseFloat(item.unitPrice) || 0)) * item.qty)}</td>
            <td>
              <button class="btn btn-danger btn-sm btn-qty-remove" style="padding:4px 8px; display:inline-flex; align-items:center; justify-content:center;" title="Remove"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
            </td>
          </tr>
        `;
      });
