(function () {
  ...
  const App = {
    init() {
      ...
      R.register('/custom-item-prices', () => POS.CustomItemPrice.render());
      ...
      R.register('/bulk-stock-update', () => POS.BulkStockUpdate.render());
      ...
      R.register('/payment-methods', () => {
        const u = S.getCurrentUser();
        if (u && (u.role === 'admin' || u.role === 'manager')) {
          POS.PaymentMethods.render();
        } else {
          POS.Helpers.showToast('Access denied: Admin & Manager only', 'error');
          R.navigate('/dashboard');
        }
      });
      R.register('/invoice-settings', () => {
        const u = S.getCurrentUser();
        if (u && (u.role === 'admin' || u.role === 'manager')) {
          this.renderSettings();
        } else {
    ...
    _setupSidebar() {
      ...
      if (user) {
        ...
        if (role === 'cashier') {
          setDisp('nav-grp-dashboard', 'none');
          setDisp('nav-grp-reports', 'none');
          setDisp('nav-grp-coupons', 'none');
          setDisp('nav-grp-inventory', '');
          setDisp('nav-grp-bulk-stock', '');
          setDisp('nav-grp-settings-dropdown', '');
          setDisp('nav-invoice-settings', 'none');
          setDisp('nav-sub-users', 'none');
          setDisp('nav-sub-payment-methods', 'none');
          setDisp('nav-sub-gallery', '');
          ...
          setDisp('nav-grp-bulk-stock', '');
          setDisp('nav-grp-settings-dropdown', '');
          setDisp('nav-invoice-settings', '');
          setDisp('nav-sub-users', 'none');
          setDisp('nav-sub-payment-methods', '');
          setDisp('nav-sub-gallery', '');
        } else {
          setDisp('nav-grp-dashboard', '');
          setDisp('nav-grp-reports', '');
          setDisp('nav-grp-coupons', '');
          setDisp('nav-grp-inventory', '');
          setDisp('nav-grp-bulk-stock', '');
          setDisp('nav-grp-settings-dropdown', '');
          setDisp('nav-invoice-settings', '');
          setDisp('nav-sub-users', '');
          setDisp('nav-sub-payment-methods', '');
          setDisp('nav-sub-gallery', '');
        }
      }
    ...
    _setupSettings() {
      const btn = document.getElementById('nav-invoice-settings') || document.getElementById('nav-settings');
      if (btn) {
        btn.addEventListener('click', (e) => {
          e.preventDefault();
          POS.Router.navigate('/invoice-settings');
        });
      }
    },
      ...
      const H = POS.Helpers;
        ...
        mc.innerHTML = `
          <div class="page-header">
            <div>
              <h2 class="page-title">System & Printing Settings</h2>
          ...
          <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px; max-width:1150px; align-items:start;" class="fade-in">
            ...
            <div class="card">
              <div class="card-header">
                <h3>Barcode Label Template Defaults</h3>
              </div>
              <div class="card-body" style="display:flex; flex-direction:column; gap:16px;">
                <div class="form-group">
